...
- *Workspace scope.* Earlier chi versions carried a workspace guard that blocked the file tools (read/write/edit) outside the session's repo. It was narrower than it read — it never covered the shell, so an agent could still `cat` a file outside the workspace — and that asymmetry surfaced in course dogfooding when the guard blocked pi's own installed docs while the shell read them fine. Upstream removed the guard, and the fleet's pin has followed: workspace scoping is now a stated convention (AGENTS.md) plus provenance, not an enforcement layer. Cross-repo work still goes through chi's own repo switching, which is what keeps provenance landing on the repo that was actually edited.
...
chi and jilog started deliberately parallel: two independent consumers of the same Pi JSONL, one for memory and provenance, one for learning signals. A first integration lane now joins them nightly, without coupling either tool ([#5](https://github.com/Joi/student-stack/issues/5)): a mechanical generator reads the sessions, chi's memory rollups, chi's provenance refs, and the jilog digests, and writes three vault pages — `learning/sessions-view.md` (per-session: what ran, what it cost, what jilog flagged, which commits it produced), `learning/sessions-view.json` (the same joined data for any web viewer or agent), and `learning/teachings.md` (recurring corrections and patterns, which the vault's AGENTS.md tells every new session to read first — the digest-feeds-memory loop in its simplest honest form). chi and jilog themselves stay independent; the join lives in plain files the student owns.
